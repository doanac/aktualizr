...
void LiteClient::notifyDownloadStarted(const Uptane::Target &t) {
  if (!config.tls.server.empty()) {
    report_queue->enqueue(std_::make_unique<EcuDownloadStartedReport>(primary_serial, t.correlation_id()));
  }
}

void LiteClient::notifyDownloadFinished(const Uptane::Target &t, bool success) {
  if (!config.tls.server.empty()) {
    report_queue->enqueue(std_::make_unique<EcuDownloadCompletedReport>(primary_serial, t.correlation_id(), success));
  }
}

void LiteClient::notifyInstallStarted(const Uptane::Target &t) {
  if (!config.tls.server.empty()) {
    report_queue->enqueue(std_::make_unique<EcuInstallationStartedReport>(primary_serial, t.correlation_id()));
  }
}
...
void LiteClient::notifyInstallFinished(const Uptane::Target &t, data::ResultCode::Numeric rc) {
  if (!config.tls.server.empty()) {
    if (rc == data::ResultCode::Numeric::kNeedCompletion) {
      report_queue->enqueue(std_::make_unique<EcuInstallationAppliedReport>(primary_serial, t.correlation_id()));
    } else if (rc == data::ResultCode::Numeric::kOk) {
      report_queue->enqueue(std_::make_unique<EcuInstallationCompletedReport>(primary_serial, t.correlation_id(), true));
    } else {
      report_queue->enqueue(std_::make_unique<EcuInstallationCompletedReport>(primary_serial, t.correlation_id(), false));
    }
  }
}
